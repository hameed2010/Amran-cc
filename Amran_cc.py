...
def logo():
    figlet = Figlet(font="standard").renderText("Amran cc")
    return (gn + figlet)
print (logo())
print (bl + "[-] Powered by Black Amran Security Team ")
print (gn + "[+] Made By Amran Team")
print (cy + "[=] Amran CC Tools Version : 1.1")
...
def genscard():
    ...
    return (gn + "[-] Brand : %s\n[-] Card Number : %s\n[-] Bank : %s\n[-] Name : %s\n[-] Address : %s\n[-] Country : %s\n[-] Money Range : %s\n[-] CVV : %s\n[-] Expiry : %s\n[-] Pin : %s\n============================\n[*] Telegram Channel : @Zak7711\n Amran Team For Information Technology" % (card['IssuingNetwork'] , card['CardNumber'] , card['Bank'] , card['Name'] , card['Address'] , card['Country'] , card['MoneyRange'] , card['CVV'] , card['Expiry'] , card['Pin']) + cv)
...
if opr == "1":
    ...
    print (mag + "[$] Telegram Channel : @Zak7711" + cv)